...
pacman -Sy efitools sbsigntools patch archlinux-keyring sbctl jq base-devel
...
git clone https://github.com/GrapheneOS/hardened_malloc ~/hmalloc && cd ~/hmalloc && make && mv out/libhardened_malloc.so /mnt/usr/local/lib/ && chmod 755 /mnt/usr/local/lib/libhardened_malloc.so && cd -
echo "/usr/local/lib/libhardened_malloc.so" > /mnt/etc/ld.so.preload
