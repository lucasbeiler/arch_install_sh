...
SERVICES_TO_ENABLE="apparmor iptables ip6tables iwd systemd-networkd systemd-resolved chronyd systemd-homed fstrim.timer mydenyusb da-lockout-clear-tpm"
KERNEL_BOOT_PARAMS="apparmor=1 security=apparmor lsm=landlock,lockdown,yama,integrity,apparmor,bpf slab_nomerge init_on_alloc=1 init_on_free=1 page_alloc.shuffle=1 oops=panic ${CPU_VENDOR}_iommu=on iommu=force iommu.strict=1 iommu.passthrough=0 vsyscall=none pti=on spectre_v2=on mds=full,nosmt efi=disable_early_pci_dma spec_store_bypass_disable=on tsx=off tsx_async_abort=full,nosmt l1tf=full,force nosmt=force kvm.nx_huge_pages=force randomize_kstack_offset=on debugfs=off libata.force=3.00:disable,3.00:norst extra_latent_entropy modprobe.blacklist=thunderbolt sysctl.kernel.deny_new_usb=1 "

PKGS_TO_IGNORE="--ignore sudo" # Keep this variable empty if you have no packages to ignore.

# Core.
PACSTRAP_PACKAGES="base ${BASE_DEVEL_WITHOUT_SUDO} ${KERNEL} ${KERNEL}-headers ${CPU_VENDOR}-ucode man-db iwd btrfs-progs chrony sbsigntools opendoas apparmor dracut binutils elfutils tpm2-tools sbctl linux-firmware"

if [[ ! -f "/tmp/arch_install_sh_devmode" ]]; then
  # Development tools.
  PACSTRAP_PACKAGES+=" neovim jq clang go rust python-pip python-requests helix code lapce qtcreator patchutils nodejs npm nano cargo"
  
  # General system utils.
  PACSTRAP_PACKAGES+=" wireguard-tools pacman-contrib zip net-tools gptfdisk openssh alacritty pamixer uutils-coreutils exa neofetch git unzip unrar pipewire-jack power-profiles-daemon python-gobject sof-firmware wireplumber pipewire-pulse pavucontrol"
  
  # Browsers, chat applications and fonts.
  PACSTRAP_PACKAGES+=" chromium gomuks irssi ttf-ubuntu-font-family ttf-opensans ttf-fantasque-sans-mono ttf-font-awesome otf-font-awesome "
  
  # Cloud, infrastructure and virtualization.
  PACSTRAP_PACKAGES+=" ansible kubectl skaffold helm kustomize docker-buildx istio vagrant qemu-base dnsmasq libvirt bridge-utils docker docker-compose terraform minikube azure-cli aws-cli helm k9s "
  
  # Multimedia and document viewers.
  PACSTRAP_PACKAGES+=" zathura zathura-pdf-mupdf tesseract-data-por ncspot"
  
  # Security tools.
  PACSTRAP_PACKAGES+=" checksec bubblewrap-suid peda valgrind gef radare2 ropgadget pwndbg binwalk arti openbsd-netcat sqlmap rustscan nmap arch-repro-status tcpdump wireshark-qt"
  
  # User interface and GUI.
  # PACSTRAP_PACKAGES+=" plasma-desktop flameshot qt6-multimedia-ffmpeg grim xdg-desktop-portal xdg-desktop-portal-kde systemsettings xdg-desktop-portal-kde kpipewire plasma-pa kscreen plasma-workspace-wallpapers dolphin vulkan-validation-layers vulkan-icd-loader vulkan-headers vulkan-tools"
