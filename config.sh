...
SERVICES_TO_ENABLE="apparmor iptables ip6tables iwd systemd-networkd systemd-resolved docker chronyd systemd-homed mydenyusb da-lockout-clear-tpm"
...
if [[ ! -f "/tmp/arch_install_sh_devmode" ]]; then
  # Development tools.
  PACSTRAP_PACKAGES+=" jq yq npm clang go rust python-pip python-requests python-beautifulsoup4 python-html5lib dbeaver helix zed lapce neovim code patchutils nano cargo "
  ...
  PACSTRAP_PACKAGES+=" kubectl age sops fluxcd skaffold helm kustomize docker-buildx istio vagrant qemu-base dnsmasq libvirt bridge-utils docker docker-compose opentofu pulumi minikube azure-cli aws-cli helm k9s "
