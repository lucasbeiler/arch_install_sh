...
SERVICES_TO_ENABLE="apparmor iptables iwd bluetooth dnscrypt-proxy ntpd-rs systemd-homed mydenyusb da-lockout-clear-tpm"
...
if [[ ! -f "/tmp/arch_install_sh_devmode" ]]; then
  ...
  PACSTRAP_PACKAGES+=" wireguard-tools dnscrypt-proxy flatpak eza android-tools pacman-contrib zip net-tools gptfdisk openssh pamixer uutils-coreutils fastfetch git unzip unrar pipewire-jack power-profiles-daemon python-gobject sof-firmware wireplumber pipewire-pulse pavucontrol"
  ...
  PACSTRAP_PACKAGES+=" plasma-desktop bluedevil flameshot qt6-multimedia-ffmpeg grim xdg-desktop-portal libnotify xdg-desktop-portal-kde systemsettings xdg-desktop-portal-kde kpipewire plasma-pa kscreen plasma-workspace-wallpapers dolphin vulkan-validation-layers vulkan-icd-loader vulkan-headers vulkan-tools"
